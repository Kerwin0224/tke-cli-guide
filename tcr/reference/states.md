...
| `Running` | 运行中 | 创建完成 | 全部操作（推送 / 拉取 / 配置访问 / 生命周期） | 否 |
| `Unhealthy` | 状态异常 | 后端健康检查失败 | 查 `Conditions.Reason` / 提工单 | 否 |
| `FailedCreated` | 创建失败 | 创建流程失败 | 删实例重试或提工单 | 是 |
| `FailedUpdated` | 更新失败 | 变配/更新失败 | 按 `Reason` 处理或提工单 | 否 |
| `Bucket-Error` | 存储桶异常 | 底层 COS 异常 | 提工单 | 否 |
| `Isolate` | 待回收（欠费等） | 欠费/隔离策略 | 续费恢复（以控制台/账单为准） | 否 |
| `Deleting` | 删除中 | `DeleteInstance` | 等待 | 是 |
| `DeleteBucketFailed` | 删实例时存储桶失败 | 删除流程 | 提工单 | 是 |
| `DeleteFailed` | 实例删除失败 | 删除流程 | 提工单 | 是 |
