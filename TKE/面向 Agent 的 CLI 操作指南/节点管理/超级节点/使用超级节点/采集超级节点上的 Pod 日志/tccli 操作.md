...
# expected: TotalCount >= 1，LifeState == "running"
```
...
{
    "TotalCount": 1,
    "NodePoolSet": [{"NodePoolId": "np-example", "LifeState": "running"}]
}
```
...
| 安装日志采集组件 | `tke InstallAddon --AddonName tke-log-agent` | 是 |
| 安装日志 Agent | `tke InstallLogAgent` | 是 |
...
`InstallLogAgent` 在集群节点上部署日志采集 Agent DaemonSet，负责实际采集容器标准输出和文件日志。
...
tccli tke CreateCLSLogConfig --region <Region> \
    ...
    --ClusterType tke \
    --LogsetId <LogsetId> \
    --LogConfig "$(cat log-config.json | python3 -c 'import sys,json; print(json.dumps(json.dumps(json.load(sys.stdin))))' 2>/dev/null || cat log-config.json)"
# expected: exit 0, 返回 RequestId
```
...
{
    "TotalCount": 1,
    "NodePoolSet": [{"NodePoolId": "np-example", "Name": "<PoolName>", "LifeState": "running"}]
}
```
...
- [新建超级节点](../../../../集群配置/节点管理/超级节点/使用超级节点/新建超级节点/tccli%20操作.md) — 创建超级节点池和虚拟节点
- [调度 Pod 至超级节点](../../../../集群配置/节点管理/超级节点/超级节点调度说明/调度%20Pod%20至超级节点/tccli%20操作.md) — 将工作负载调度到超级节点
- [指定资源规格](../../../../集群配置/节点管理/超级节点/使用超级节点/指定资源规格/tccli%20操作.md) — 为超级节点 Pod 指定资源
- [超级节点上支持运行 DaemonSet](../../../../集群配置/节点管理/超级节点/使用超级节点/超级节点上支持运行%20DaemonSet/tccli%20操作.md) — DaemonSet 在超级节点上的运行方式
