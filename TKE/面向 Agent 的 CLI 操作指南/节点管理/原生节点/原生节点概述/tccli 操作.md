...
原生节点是 TKE 推出的新一代节点类型，基于 CXM 机型，支持内存压缩、Pod 原地升降配、故障自愈等高级特性。原生节点通过**原生节点池**统一管理，创建方式包括控制台、Kubernetes API 和云 API（`CreateClusterNodePool` 指定 `Type="Native"`）。
...
# expected: exit 0，NodePoolSet 中每项含 NodePoolType 字段
```
...
| 现象 | 诊断 | 根因 | 修复 |
|------|------|------|------|
| 节点池 Type 为 "Regular" | 检查创建时使用的 `CreateClusterNodePool` 参数 | 创建时 Type 未指定为 "Native"，默认为普通节点池 | 普通节点池不具备原生节点能力。如需原生节点，创建新的 Type="Native" 节点池 |
