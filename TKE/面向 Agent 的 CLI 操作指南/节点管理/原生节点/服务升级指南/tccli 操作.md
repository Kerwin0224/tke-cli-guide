...
tccli tke DescribeClusterInstances --region <Region> \
    --ClusterId CLUSTER_ID \
    --InstanceRole "WORKER"
# expected: 返回节点列表含 InstanceId 和当前版本信息
...
| 查看可用运行时版本 | `DescribeSupportedRuntime` | 是 |
...
# expected: RuntimeSet 中返回 containerd 可用版本
```
...
# 查看业务 Pod 的分布情况（将 Kubeconfig 写入 KUBECONFIG_PATH 后执行）
kubectl --kubeconfig KUBECONFIG_PATH get pods -o wide --all-namespaces \
    --field-selector spec.nodeName=NODE_NAME
# expected: 列出该节点上所有 Pod，确认均为多副本或有容忍的 workload

# 查看 PodDisruptionBudget（确保滚动升级不违反 PDB）
kubectl --kubeconfig KUBECONFIG_PATH get pdb --all-namespaces
# expected: 列出所有 PDB 策略，确认升级操作不会导致可用副本数低于 PDB 阈值
```

### 步骤 4：触发滚动升级

通过修改节点池的 `ImageId` 触发滚动升级，新镜像版本会逐台替换存量节点：

`upgrade-nodepool.json`：
...
| `TARGET_IMAGE_ID` | 目标操作系统镜像 ID | 原生节点兼容的 TencentOS 镜像 | `tccli tke DescribeOSImages --region <Region>` |
| `CXM_INSTANCE_TYPE` | CXM 机型 | 当前节点池使用的机型 | `DescribeClusterNodePoolDetail` -> `InstanceTypes` |
| `SUBNET_ID` | 子网 ID | 节点池当前子网 | `DescribeClusterNodePoolDetail` -> `SubnetIds` |
...
tccli tke DescribeClusterNodePoolDetail --region <Region> \
    --ClusterId CLUSTER_ID \
    --NodePoolId NODE_POOL_ID
# expected: LifeState 从 "updating" 变为 "normal"；Normal 数量逐步恢复，Joining/Initializing 数量减少至 0
```
...
| 节点数恢复 | 同上 -> `NodeCountSummary` 各 Normal 计数 | = 期望节点数 |
...
> **注意**：升级操作本身会触发节点重建，旧版本节点会被自动替换。如升级过程中遇到问题，可通过 `ModifyClusterNodePool` 回滚 `ImageId` 到旧镜像版本。
...
| 现象 | 诊断 | 根因 | 修复 |
|------|------|------|------|
| `ModifyClusterNodePool` 返回 `InvalidParameterValue.ImageId` | `tccli tke DescribeOSImages --region <Region>` 检查镜像是否存在 | 填写的 `ImageId` 不存在或不属于当前地域 | 用正确镜像 ID 重新调用；`tccli tke DescribeOSImages --region <Region>` 查询可用镜像 |
| `ModifyClusterNodePool` 返回 `InvalidParameterValue.SubnetId` | 检查 JSON 中 `SubnetIds` 数组 | 子网 ID 已删除或不属于集群 VPC | `tccli vpc DescribeSubnets --region <Region>` 查询可用子网 |
...
| 现象 | 诊断 | 根因 | 修复 |
|------|------|------|------|
| 节点池 `LifeState=updating` 超过 30 分钟未完成 | `tccli tke DescribeClusterNodePoolDetail --region <Region> --ClusterId CLUSTER_ID --NodePoolId NODE_POOL_ID` 查看 NodeCountSummary 中各状态数量 | 新节点创建失败、镜像兼容性问题、子网 IP 不足 | 检查 `Abnormal` 节点数量 -> 查看节点实例 `FailedReason` -> 如果是 IP 不足，释放子网 IP 或换子网；如果是镜像问题，换用已验证的镜像 ID |
| 升级后新节点 NotReady | `kubectl --kubeconfig KUBECONFIG_PATH describe node NODE_NAME` 查看 Conditions | kubelet/containerd 版本与新镜像不兼容，或内核参数不匹配 | 回滚 ImageId 到旧版本；如为 KubeletArgs 问题，同步更新 Management 参数 |
| 业务 Pod 无法调度到新节点 | `kubectl --kubeconfig KUBECONFIG_PATH describe pod POD_NAME` 查看 Events | 新节点缺少 Pod 所需的 nodeSelector 标签或污点/容忍不匹配 | 检查新节点的 labels 和 taints 与旧节点是否一致；不一致需用 `ModifyClusterNodePool` 补齐配置 |

## 下一步

- [Management 参数介绍](https://cloud.tencent.com/document/product/457/79698) -- 升级后可调整 KubeletArgs 等运维参数
- [镜像和内核参数说明](https://cloud.tencent.com/document/product/457/124612) -- 原生节点支持的镜像和内核版本
- [原生节点生命周期](https://cloud.tencent.com/document/product/457/78200) -- 节点从创建到删除的完整生命周期
- [故障自愈规则](https://cloud.tencent.com/document/product/457/78209) -- 升级异常时的自动恢复策略
