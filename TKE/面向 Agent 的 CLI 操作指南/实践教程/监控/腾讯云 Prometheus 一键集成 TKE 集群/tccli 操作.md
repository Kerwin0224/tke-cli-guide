...
| `CreatePrometheusClusterAgent` 返回 `FailedOperation` | `tccli tke DescribeClusters --region ap-guangzhou --ClusterIds '["<ClusterId>"]' --filter "Clusters[0].ClusterStatus"` | ClusterId 错误或集群非 Running | 核对 ClusterId；等待集群 Running 后重试 |
| Grafana 无数据 | `tccli tke DescribePrometheusClusterAgents --InstanceId <InstanceId> --region ap-guangzhou` | 采集配置未勾选指标或 Agent 未 running | 勾选采集指标；等待 1-2 分钟数据上报 |
| 告警策略未触发 | `tccli tke DescribePrometheusAlertPolicy --InstanceId <InstanceId> --region ap-guangzhou` | 告警表达式错误或通知模板缺失 | 核对 PromQL 表达式；配置通知模板 |
| 绑定报地域不一致 | 同上诊断命令 | Agent 的 Region 与集群 Region 不一致 | 绑定时 Agent.Region 需与集群 Region 一致 |
