...
基于腾讯云负载均衡 CLB 提供四层与七层流量接入方案，详见[流量接入](traffic-access.md)。当前仅注册节点（专线版）支持接入 CLB，公网版暂不支持。
...
TKE 集群 Prometheus 监控服务默认支持集群中的注册节点，无需特殊配置。当前仅注册节点（专线版）支持接入 Prometheus，公网版暂不支持。
