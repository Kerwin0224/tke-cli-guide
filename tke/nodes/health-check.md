...
> ⚠️ 本文档所有 Action 属 **TKE 2022-05-01（官方当前版本）**，调用必须显式 `--version 2022-05-01`。2018-05-25 无此功能域（D29 全新领域）。不带 `--version` 会静默走 2018-05-25 报「未知 Action」（D32 准则 1）。

## 概述

节点健康检查策略（HealthCheckPolicy）按一组规则定期检测节点故障（如 kubelet 不健康、文件系统只读、内核 oops），发现故障后可按规则配置自动修复（重启 runtime / 重启 kubelet）。

健康检查是**节点级**故障自愈，区别于集群巡检（`ListClusterInspectionResults`，集群级配置扫描，归 Observability）。两者层级与任务不同，勿混。

```bash
# 集群级配置巡检（ClusterIds[] + Name 过滤；区别于本文档的节点级健康检查策略）
tccli tke ListClusterInspectionResults --version 2018-05-25 \
  --region <REGION> --ClusterIds '["<CLUSTER_ID>"]'
# expected: exit 0, 返回集群配置巡检结果（集群级，非节点级）
```

> `ListClusterInspectionResults` 是集群级配置扫描（归 Observability 域），与本文档的节点级 `HealthCheckPolicy`（2022-05-01）层级不同、版本不同（前者 2018-05-25、后者 2022-05-01）。混淆两者会调错版本报「未知 Action」。

## 决策依据

### 启用哪些规则

健康检查模板含 15 条规则，分两类（实测 `DescribeHealthCheckTemplate`，权威来源）：
...
> 参数名实测自 `tccli tke CreateHealthCheckPolicy --version 2022-05-01 --generate-cli-skeleton`（P7）。
...
> ⚠️ **地域限制（实测）**：`DescribeHealthCheckTemplate` 在 `ap-guangzhou` 返回 `UnsupportedRegion`；改用 `ap-beijing` / `ap-shanghai` / `ap-chengdu` / `ap-nanjing`。模板是全局一致的，任一支持地域查询结果相同。
...
tccli tke DescribeHealthCheckPolicyBindings --version 2022-05-01 --region <REGION> \
  ...
  --Offset 0 --Limit 20
# expected: exit 0, 返回策略与节点池/节点的绑定关系
```
...
| `AuthFailure.UnauthorizedOperation` (tke:CreateHealthCheckPolicy) | CAM 策略要求资源带特定标签（实测要求 `billing&kerwinwjyan` 标签） | 给集群加授权要求的标签，或申请 `tke:CreateHealthCheckPolicy` 权限 |
...
| Action | 类型 | 版本 | 说明 |
|:-------|:-----|:-----|:-----|
| `DescribeHealthCheckTemplate` | 验证 | 2022-05-01 | 查询规则模板（15 条），地域受限 |
| `CreateHealthCheckPolicy` | 主操作 | 2022-05-01 | 创建策略 |
| `DescribeHealthCheckPolicies` | 验证 | 2022-05-01 | 查询策略列表 |
| `DescribeHealthCheckPolicyBindings` | 验证 | 2022-05-01 | 查询策略绑定（入参 `Filter` 无 s） |
| `ModifyHealthCheckPolicy` | 主操作 | 2022-05-01 | 修改策略（覆盖式） |
| `DeleteHealthCheckPolicy` | 清理 | 2022-05-01 | 删除策略（用 `HealthCheckPolicyName`） |
