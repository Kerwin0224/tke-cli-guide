...
| ClusterType | `MANAGED_CLUSTER`（托管）/ `INDEPENDENT_CLUSTER`（独立，**已停止新建**） | 决定 Master 归谁管；新建默认走托管 |
...
|:-----|:---------|:-----------|:-----|:-----|:-----|
| MANAGED_CLUSTER（托管） | 生产与常规业务 | 腾讯云运维；不可改 Master/Etcd 规模与参数 | API/控制台升级 | 集群管理费 + 节点等云资源费 | ✅ 默认路径 |
...
- **新建**免 CVM、要编排 → ① 本文 `CreateCluster` ② [虚拟节点](../nodes/virtual-nodes.md)（两步；仅 CreateCluster 不够）；无集群容器 → [容器实例](../specialized/eks-cluster.md#创建容器实例部署-pod)；存量 EKS → [EKS](../specialized/eks-cluster.md)（新建入口已关闭）
...
| 5. 运行时 | [配置集群属性与运行时](configure.md) + [升级集群版本](upgrade.md) | 集群"怎么跑"：版本升级/运行时配置/Master 组件/OS 镜像/自定义参数（22 Action 独立概念） | ClusterVersion/运行时/Master 配置变更 |
