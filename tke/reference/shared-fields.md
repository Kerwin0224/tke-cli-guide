...
`Filter` 出现在所有 `Describe*` Action（35 个），用于按属性过滤结果。

| 字段 | 类型 | 说明 |
|------|------|------|
| Name | string | 属性名（如 `cluster-id`/`cluster-name`/`vpc-id`） |
| Values | array | 属性值列表，同 Filter 内多值是 OR，多 Filter 间是 AND |

```bash
# 多 Filter AND, 同 Filter 多 Value OR
tccli tke DescribeClusters --region <REGION> \
  --Filters '[
    {"Name":"cluster-name","Values":["prod"]},
    {"Name":"vpc-id","Values":["vpc-aaa","vpc-bbb"]}
  ]'
# expected: Clusters[] 只含名称含 prod 且 VPC 为 vpc-aaa/vpc-bbb 的集群
```

> 各 Describe Action 支持的 Filter `Name` 不同，用 `tccli tke <Action> help --detail` 查该 Action 的合法 Name。传错 Name 返回 `InvalidParameter`。
...
| KeyIds | array | 密钥 ID 列表 |
| KeepImageLogin | string | 保持镜像原始登录设置（与 Password/KeyIds 互斥） |

```bash
--LoginSettings '{"Password":"<PASSWORD>","KeyIds":["skey-xxx"]}'
```

> Password 与 KeepImageLogin 互斥；KeyIds 通过 `tccli cvm DescribeKeyPairs` 获取。
...
| 透传 CVM 参数 | `RunInstancesPara` 接 CVM `RunInstances` 整段 JSON 字符串 |
