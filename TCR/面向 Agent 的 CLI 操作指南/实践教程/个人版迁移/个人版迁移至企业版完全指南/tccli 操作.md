...
通过个人版域名兼容功能，业务集群无需修改镜像地址即可拉取已迁移到企业版的镜像。参见[使用个人版域名访问企业版实例](../使用个人版域名访问企业版实例/tccli%20操作.md)。
...
> TKE 集群推荐安装 TCR 插件实现免密拉取（参见[TKE 集群使用 TCR 插件内网免密拉取容器镜像](../../TKE%20集群使用%20TCR%20插件内网免密拉取容器镜像/tccli%20操作.md)），无需手动配置 `imagePullSecret`。
...
> **计费警告**：TCR 企业版实例按规格持续计费。迁移完成后如仅用于验证，请及时[销毁退还实例](../../../../操作指南/销毁退还实例/tccli%20操作.md)。个人版源镜像建议保留一段时间作为备份。
>
