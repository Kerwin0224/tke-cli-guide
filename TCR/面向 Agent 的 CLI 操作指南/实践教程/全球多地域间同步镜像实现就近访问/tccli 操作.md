...
> **结合自定义域名：** TCR 企业版支持自定义域名，结合 DNS 服务可实现多个实例共用同一域名，增强就近访问的灵活性。参见[配置自定义域名](../../操作指南/访问配置/访问域名配置/配置自定义域名/tccli%20操作.md)。
...
> 实例创建详情参见[创建企业版实例](../../操作指南/创建企业版实例/tccli%20操作.md)。以下提供 CLI 方式快速创建，含最小和增强两种配置。
...
TCR 企业版支持自定义域名，结合 DNS 就近解析实现多实例统一入口。详见[配置自定义域名](../../操作指南/访问配置/访问域名配置/配置自定义域名/tccli%20操作.md)。
...
- [同实例多地域复制镜像](../../操作指南/镜像分发/同实例多地域复制镜像/tccli%20操作.md) — 高级版实例复制详细指南
- [跨实例（账号）同步镜像](../../操作指南/镜像分发/跨实例（账号）同步镜像/tccli%20操作.md) — TCR 实例间同步规则管理
- [从自建 Harbor 同步镜像到 TCR 企业版](https://cloud.tencent.com/document/product/1141/44970) — Harbor 迁移至 TCR
- [混合云下的多平台镜像数据同步复制](../混合云下的多平台镜像数据同步复制/tccli%20操作.md) — 跨平台同步场景
