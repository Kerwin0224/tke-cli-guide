...
- [环境准备](../../../../环境准备.md)：`tccli` 版本 >= 3.1.x，地域与凭据已配置
- 已成功 [购买企业版实例](../../创建企业版实例/tccli%20操作.md)（企业版 `basic`/`standard`/`premium`），实例状态为 `Running`
...
|---------|---------------|--------------|------|
| 容器镜像 — 仓库管理 | `CreateRepository` / `DeleteRepository` | — | [管理镜像仓库](../../镜像创建/管理镜像仓库/tccli%20操作.md) |
| 容器镜像 — 版本管理 | `DescribeImages` / `DescribeImageManifests` | `docker push`/`pull` | 同上 + [自动删除镜像版本](../../镜像清理/自动删除镜像版本/tccli%20操作.md) |
| 容器镜像 — 版本不可变 | `CreateImmutableTagRules` | — | [镜像版本不可变](../../镜像安全/镜像版本不可变/tccli%20操作.md) |
| Helm Chart — OCI 模式 | `DescribeImages`（Chart 以 OCI 制品形式出现） | `helm push`/`pull oci://` | [托管 Helm Chart](../托管%20Helm%20Chart/tccli%20操作.md) |
| Helm Chart — ChartMuseum | 企业版内置 ChartMuseum API | `helm repo add`/`push`（传统模式） | 同上 |
| 加速镜像 | `CreateImageAccelerateTask` | `docker`/`nerdctl` 配合 Nydus | [按需加载容器镜像](../../镜像分发/按需加载容器镜像/tccli%20操作.md) |

## 操作步骤

本文为概念概述页，无可独立执行的 tccli 命令操作。具体操作步骤请进入以下子页面：

- [管理镜像仓库](../../镜像创建/管理镜像仓库/tccli%20操作.md) — 创建、查看、删除镜像仓库（page_id `41811`）
- [托管 Helm Chart](../托管%20Helm%20Chart/tccli%20操作.md) — 推送与管理 Helm Chart 制品（page_id `41944`）
- [按需加载容器镜像](../../镜像分发/按需加载容器镜像/tccli%20操作.md) — 将镜像转换为 OCI 加速格式（page_id `53928`）
...
- [托管 Helm Chart](../托管%20Helm%20Chart/tccli%20操作.md) — 推送与管理 Helm Chart 制品（OCI 模式 + ChartMuseum 模式）（page_id `41944`）
- [管理镜像仓库](../../镜像创建/管理镜像仓库/tccli%20操作.md) — 创建与管理容器镜像仓库（page_id `41811`）
- [按需加载容器镜像](../../镜像分发/按需加载容器镜像/tccli%20操作.md) — 将镜像转换为 OCI 加速格式（page_id `53928`）
- [容器镜像安全扫描](../../镜像安全/容器镜像安全扫描/tccli%20操作.md) — 扫描镜像漏洞与恶意文件
- [自动删除镜像版本](../../镜像清理/自动删除镜像版本/tccli%20操作.md) — 配置版本保留策略
