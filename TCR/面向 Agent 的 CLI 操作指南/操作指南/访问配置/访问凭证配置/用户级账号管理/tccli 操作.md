...
> 与服务级账号（[服务级账号管理](../服务级账号管理/tccli%20操作.md)）不同，用户级账号 Token 面向人机身份，**不具备命名空间级权限隔离**。如需细粒度权限控制，请使用服务级账号。
...
- [服务级账号管理](../服务级账号管理/tccli%20操作.md)（page_id `89137`）——创建命名空间级权限隔离的机器人账号
- [基于 CAM 管理子账号权限](../访问权限配置/基于%20CAM%20管理子账号权限/tccli%20操作.md)——管理子账号的 TCR 操作权限
- [管理命名空间](../../镜像创建/管理命名空间/tccli%20操作.md)——创建命名空间后使用 Token 推送镜像
- [获取实例访问域名](../../创建企业版实例/tccli%20操作.md)——确认 `PublicDomain` 用于 `docker login`
