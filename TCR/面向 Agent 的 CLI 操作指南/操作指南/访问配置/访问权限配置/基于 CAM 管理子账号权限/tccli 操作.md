...
- 已完成 [环境准备](../../../../环境准备.md)，安装并配置 `tccli`（默认地域 `ap-guangzhou`）。
...
- [访问权限管理概述](../访问权限管理概述/tccli%20操作.md)（page_id `40718`）
- [服务级账号管理](../../访问凭证配置/服务级账号管理/tccli%20操作.md) — TCR 实例级服务账号（与 CAM 子账号互补）
- [创建企业版实例](../../../创建企业版实例/tccli%20操作.md)（page_id `51110`）— 实例创建后配置访问权限
