...
- 配合 [TKE 集群使用 TCR 插件内网免密拉取](../../../../实践教程/TKE%20集群使用%20TCR%20插件内网免密拉取容器镜像/tccli%20操作.md) 实现 Kubernetes 集群内网免密镜像拉取

## 前置条件

- [环境准备](../../../../环境准备.md)：`tccli` 版本 >= 3.1.x，地域与凭据已配置
- 已成功 [购买企业版实例](../../../创建企业版实例/tccli%20操作.md)（`--RegistryType basic`/`standard`/`premium`），实例状态为 `Running`
- **公网访问控制**：无需额外前提
...
| 控制台操作 | CLI 命令 | 页面 |
|-----------|----------|------|
| 开启/关闭公网访问入口 | `ManageExternalEndpoint` | [配置公网访问控制](../配置公网访问控制/tccli%20操作.md) |
...
| 控制台操作 | CLI 命令 | 页面 |
|-----------|----------|------|
| 新建/删除内网访问链路 | `ManageInternalEndpoint` | [配置内网访问控制](../配置内网访问控制/tccli%20操作.md) |
...
- [配置公网访问控制](../配置公网访问控制/tccli%20操作.md) — 开启/关闭公网入口、管理白名单策略（page_id `41837`）
- [配置内网访问控制](../配置内网访问控制/tccli%20操作.md) — 接入私有网络、管理内网私有域解析（page_id `41838`）
...
- [配置公网访问控制](../配置公网访问控制/tccli%20操作.md)（page_id `41837`）— 开启/关闭公网入口、管理白名单策略
- [配置内网访问控制](../配置内网访问控制/tccli%20操作.md)（page_id `41838`）— 接入私有网络、管理内网私有域解析
- [访问权限管理概述](../../访问权限配置/访问权限管理概述/tccli%20操作.md)（page_id `40718`）— 理解用户级/服务级账号权限管理
- [TKE 集群使用 TCR 插件内网免密拉取容器镜像](../../../../实践教程/TKE%20集群使用%20TCR%20插件内网免密拉取容器镜像/tccli%20操作.md) — 实战：K8s 集群内网免密拉取
