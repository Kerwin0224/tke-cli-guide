...
| 现象 | 诊断 | 根因 | 修复 |
|------|------|------|------|
| `CreateRepository` 返回 `ResourceNotFound.Namespace` | `tccli tcr DescribeNamespaces --RegistryId '<RegistryId>' --region <Region>` 确认命名空间是否存在 | 指定的 `NamespaceName` 不存在，或拼写错误 | 先 [创建命名空间](../管理命名空间/tccli%20操作.md)，确认 `NamespaceName` 拼写正确 |
...
- [管理命名空间](../管理命名空间/tccli%20操作.md)（page_id `41803`）
- [同实例多地域复制镜像](../../镜像分发/同实例多地域复制镜像/tccli%20操作.md)（page_id `52095`）
- [触发器（Webhook）](../../DevOps/触发器（Webhook）/tccli%20操作.md)
- [企业版快速入门](../../../快速入门/企业版快速入门/tccli%20操作.md)
