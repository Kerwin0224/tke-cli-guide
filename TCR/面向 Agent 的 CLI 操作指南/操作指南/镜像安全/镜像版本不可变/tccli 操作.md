...
- 已 [创建企业版实例](../../创建企业版实例/tccli%20操作.md)，实例状态为 `Running`。
- 已配置 `tccli` 凭证（参见 [环境准备](../../../环境准备.md)）。
- 实例内已有目标命名空间（参见 [管理命名空间](../../镜像创建/管理命名空间/tccli%20操作.md)）。
...
| `CreateImmutableTagRules` 返回 `ResourceNotFound.TcrResourceNotFound`: "namespace ... not found" | `tccli tcr DescribeNamespaces --RegistryId <RegistryId> --region <Region>` 确认命名空间存在 | `NamespaceName` 错误或命名空间未创建 | 核对命名空间名称，参考 [管理命名空间](../../镜像创建/管理命名空间/tccli%20操作.md) 创建 |
...
- [容器镜像安全扫描](../容器镜像安全扫描/tccli%20操作.md) — 对镜像进行漏洞扫描与修复
- [容器镜像签名](../容器镜像签名/tccli%20操作.md) — 镜像签名与验签
- [按策略自动删除镜像版本](../../镜像清理/自动删除镜像版本/tccli%20操作.md) — 自动清理过期镜像版本
- [触发器（Webhook）](../../DevOps/触发器（Webhook）/tccli%20操作.md) — 镜像推送/删除自动触发 CI/CD
- [环境准备](../../../环境准备.md) — 返回 TCR 工具链入口
