...
> **注意**：
> - GC 过程仅清理 COS 存储层中无引用的 blob。对于仍被镜像 Tag 引用的数据、或已被 [版本保留规则](../自动删除镜像版本/tccli%20操作.md) 删除 Tag 但未执行 GC 的 blob，需先确保无引用后再触发 GC。
> - 执行 GC 期间不影响正常镜像推拉。
...
- 建议在执行 GC 前完成需要清理的镜像 Tag 删除操作（参见 [自动删除镜像版本](../自动删除镜像版本/tccli%20操作.md)），确保目标 blob 已无引用。
...
- [自动删除镜像版本](../自动删除镜像版本/tccli%20操作.md) -- 配置版本保留规则，定期清理过期 Tag
- [管理镜像仓库](../../镜像创建/管理镜像仓库/tccli%20操作.md) -- 删除不需要的镜像仓库及 Tag
