...
> ⚠️ **控制台是固有边界**：CAM 根凭证（SecretId/SecretKey）的**首次获取**须经腾讯云控制台/浏览器——腾讯云不允许用 TCCLI 自举创建 API 密钥（`tccli auth login` 也触发浏览器登录）。凭证首次获取须经控制台一次性操作，无法纯 CLI 闭环；这是腾讯云平台边界。配好凭证后，后续所有 TKE/TCR 操作均可纯 CLI 完成。
...
| `TCR_QCSRole` | （TCR 服务） | 镜像签名需 KMS | 签名失败 | [镜像签名](../tcr/security/signing.md#服务角色tcrkms) |
...
tccli tcr DescribeRegions --profile <PROFILE_NAME> --filter "TotalCount" --output text
